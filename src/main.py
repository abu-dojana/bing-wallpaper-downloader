...
from PyQt5.QtWidgets import QSystemTrayIcon, QAction, QMenu, QDialog, QVBoxLayout, QHBoxLayout, QLabel, QComboBox, QSpinBox, QPushButton, QFileDialog, QLineEdit
...
from config.settings import WALLPAPER_TYPES, RESOLUTION_OPTIONS, WALLPAPER_PROVIDERS, DEFAULT_WALLPAPER_PROVIDER
...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        provider_layout.addWidget(QLabel('Wallpaper Provider:'))
        self.provider_combo = QComboBox()
        self.provider_combo.addItems(WALLPAPER_PROVIDERS)
        self.provider_combo.setCurrentText(self.config.get('wallpaper_provider', DEFAULT_WALLPAPER_PROVIDER))
        self.provider_combo.currentTextChanged.connect(self.provider_changed)
        provider_layout.addWidget(self.provider_combo)
        layout.addLayout(provider_layout)

        # API Key input (initially hidden)
        self.api_key_widget = QtWidgets.QWidget()
        self.api_key_layout = QHBoxLayout(self.api_key_widget)
        self.api_key_layout.addWidget(QLabel('API Key:'))
        self.api_key_edit = QLineEdit()
        self.api_key_edit.setText(self.config.get('api_keys', {}).get(self.provider_combo.currentText(), ''))
        self.api_key_edit.setEchoMode(QLineEdit.Password)  # Mask the API key
        ...
        self.provider_changed(self.provider_combo.currentText())
    ...
    def provider_changed(self, provider):
        """Handle provider change and update API key visibility"""
        if provider in self.config.get('api_keys', {}):
            self.api_key_widget.setVisible(True)
            self.api_key_edit.setText(self.config['api_keys'][provider])
        else:
...
class WallpaperApp:
    ...
    # Update the download_wallpaper method in WallpaperApp
    ...
    def download_wallpaper(self, wallpaper_url=None):
        try:
            ...
            # If URL not provided, fetch it using the selected provider
            if not wallpaper_url:
                provider_name = settings.get('wallpaper_provider', 'Bing')
                ...
                if provider.requires_api_key:
                    ...
                    if not api_key:
                        self.tray_icon.showMessage(
                            "API Key Missing",
                            f"{provider_name} requires an API key. Please add it in Settings.",
                ...
                # Fetch wallpapers from provider
                wallpapers = provider.fetch_wallpapers(
                    count=1, 
                    category=wallpaper_type,
                    resolution=resolution,
                    api_key=api_key
                )
                
                if wallpapers:
                    wallpaper_url = wallpapers[0]['url']
                else:
                    wallpaper_url = None
